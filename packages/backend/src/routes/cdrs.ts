...
// ── Proxy recording stream from SignalWire (browser can't auth directly) ──
router.get("/customer/recordings/:cdrId", async (req: Request, res: Response) => {
  try {
    const cdr = db.prepare("SELECT recording_url, call_sid, organization_id FROM cdrs WHERE id = ? AND organization_id = ?").get(req.params.cdrId, req.user!.organizationId) as any;
    if (!cdr?.recording_url) return res.status(404).json({ error: "Recording not found" });
